...
All row/column references are anchored to v4 of the model.
"""
...
# ── File path helpers ─────────────────────────────────────────────────────────
MODEL_FILENAME = "180DC_ESCP_FinancialModel_Fall2026_v4.xlsx"
LEGACY_FILENAMES = [
    MODEL_FILENAME,
...
def _candidate_excel_paths(explicit_path: str | Path | None = None) -> list[Path]:
    """Return ordered candidate paths for the Excel workbook."""
    ...
    env_path = os.getenv("EXCEL_MODEL_PATH")
    if env_path:
...
def resolve_excel_path(path: str | Path | None = None) -> Path:
    """Resolve the Excel model path across local and Streamlit Cloud layouts."""
    for candidate in _candidate_excel_paths(path):
        if candidate.exists() and candidate.is_file():
            return candidate

    searched = "\n".join(f"- {candidate}" for candidate in _candidate_excel_paths(path))
    raise FileNotFoundError(
        "Excel model not found. Searched these locations:\n"
        f"{searched}\n\n"
        "Add the workbook to the repository either in the project root or in a 'data/' folder, "
        f"or set EXCEL_MODEL_PATH to point to '{MODEL_FILENAME}'."
    )


XLSX_FILE = resolve_excel_path()
...
def load_model(path: str | Path | None = None) -> dict:
    ...
    Returns a nested dict: {assumptions, pl, kpis}.
    """
    resolved_path = resolve_excel_path(path)
...
@st.cache_data(ttl=60)
def load_projects(path: str | Path | None = None) -> list:
    resolved_path = resolve_excel_path(path)
    wb = load_workbook(resolved_path, data_only=True)
    ws = wb["Project Tracker"]
    projects = []
    for row in PROJECT_STATUS_ROWS:
        projects.append({key: ws.cell(row=row, column=col).value for key, col in PROJECT_COLS.items()})
    wb.close()
    return projects
...
@st.cache_data(ttl=60)
def load_variance(path: str | Path | None = None) -> dict:
    resolved_path = resolve_excel_path(path)
    wb = load_workbook(resolved_path, data_only=True)
    ws = wb["Variance"]
    out = {}
